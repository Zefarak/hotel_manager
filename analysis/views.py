from django.db.models import Sum, FloatField, F, Count, Avg
...
from reservations.models import Reservation, SOURCES
...
class AnalysisIncomeView(ListView):
    ...
    def get_queryset(self):
        ...
        # qs = CustomerPayment.filters_data(self.request, qs)
        return qs
    ...
    def get_context_data(self,  **kwargs):
        ...
        '''
        analysis_z_per_month = self.object_list.annotate(month=TruncMonth('date')).values('month').annotate(
            total=Sum('sum_z')).values('month', 'total').order_by('month')
        analysis_pos_per_month = self.object_list.annotate(month=TruncMonth('date')).values('month').annotate(
            total=Sum('pos')).values('month', 'total').order_by('month')
        analysis_cash_per_month = self.object_list.annotate(month=TruncMonth('date')).values('month').annotate(
...
@method_decorator(staff_member_required, name='dispatch')
class AnalysisReservationView(TemplateView):
    template_name = 'analysis/reservation_analysis.html'

    def get_context_data(self, **kwargs):
        context = super(AnalysisReservationView, self).get_context_data(**kwargs)
        context['back_url'] = reverse('analysis:homepage')
        qs = Reservation.my_query.filters_data(self.request)

        incomes = qs.filter(isDone=True, isCancel=False)
        qs_analysis = incomes.values('source').annotate(total=Sum('final_value'),
                                                        count_sum=Count('final_value'),
                                                        value=Sum('value'),
                                                        average=Avg('final_value')
                                                        ).order_by('total')

        sources = dict(Reservation._meta.get_field('source').flatchoices)
        for ele in qs_analysis:
            ele['source'] = force_text(sources[ele['source']], strings_only=True)

        total = qs.aggregate(Sum('final_value'))['final_value__sum'] if qs.exists() else 0
        isDoneQs = qs.filter(isDone=True)
        isActiveQs = qs.filter(isDone=False, isCancel=False)

        isDoneTotal = isDoneQs.aggregate(Sum('final_value'))['final_value__sum'] if isDoneQs.exists() else 0
        isActiveTotal = isActiveQs.aggregate(Sum('final_value'))['final_value__sum'] if isActiveQs.exists() else 0
        isDonePerMonth = isDoneQs.annotate(month=TruncMonth('check_out')).values('month').annotate(total=Sum('final_value')).values('month', 'total').order_by('month')
        isDonePerRoom = isDoneQs.values('room__title').annotate(
        ...
        #bills.annotate(month=TruncMonth('date_expired')).values('month').annotate(
          # total=Sum('final_value')).values('month', 'total').order_by('month')
        context['qs_analysis'] = qs_analysis
        context['isDoneTotal'] = isDoneTotal
        context['isActiveTotal'] = isActiveTotal
        context['isDonePerMonth'] = isDonePerMonth
        context['isDonePerRoom'] = isDonePerRoom
...
class CashRowView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        currency = settings.CURRENCY
        back_url = reverse('analysis:homepage')
        incomes = CustomerPayment.filters_data(self.request, CustomerPayment.objects.all()).order_by('date_expired')
...
class BalanceSheetView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # incomes
        incomes = CustomerPayment.filters_data(self.request, CustomerPayment.objects.all())

        incomes_per_month = incomes.annotate(month=TruncMonth('date')).values('month').annotate(total=Sum('value')).values('month', 'total').order_by('month')
        incomes_per_month_table = incomes.annotate(month=TruncMonth('date')).values('month')\
            .annotate(
                total=Sum('value')
                      ).order_by('month')
        # total_z = incomes.aggregate(Sum('sum_z'))['sum_z__sum'] if incomes.exists() else 0
